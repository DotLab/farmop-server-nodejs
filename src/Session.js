...
const {User, Task, Notification, Issue, createDefaultUser, createDefaultMidi, serializeUser} = require('./models');
...
module.exports = class Session {
  ...
  async onClWebTaskCreate({assigner, assignee, location, totalPp, level, title, type}, done) {
    // debug('  onClWebUserRegister', code, name, email, password);

    const now = new Date();
    await Task.create({
      assignee, assigner, location, totalPp, level, title, type,
      date: now, status: 'Current',
    });

    success(done);
  }

  async onClWebNotificationCreate({reveriverId, location, title}, done) {
    // debug('  onClWebUserRegister', code, name, email, password);

    const now = new Date();
    await Task.create({
      reveriverId, location, title,
  ...
  async onClWebFanGet({room_id}, done) {
    // debug('  onClWebUserRegister', code, name, email, password);
